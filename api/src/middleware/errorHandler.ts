  ...
  if (err instanceof Prisma.PrismaClientKnownRequestError) {
    ...
    if (err.code === "P2021" || err.code === "P2022") {
      ...
      return res.status(503).json({
        success: false,
        message:
          "Required database columns are missing (multi-school schema). Redeploy the API with DIRECT_URL set to Neon’s direct (non-pooled) connection so ensure-schema can apply School/schoolId.",
        code: err.code,
      });
    }
    if (err.code === "P2010") {
