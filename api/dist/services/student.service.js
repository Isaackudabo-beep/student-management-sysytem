...
export async function createStudent(input) {
    ...
    const mismatched = subjects.filter((s) => s.level.toUpperCase() !== schoolClass.level.toUpperCase());
    if (mismatched.length > 0) {
        throw new AppError(400, `Subjects must match class level ${schoolClass.level}: ${mismatched.map((s) => s.code).join(", ")}`);
    }
