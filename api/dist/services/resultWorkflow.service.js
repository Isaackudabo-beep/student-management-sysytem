...
function sheetStatusFromScores(enrolled, statuses) {
    if (enrolled === 0)
        return "PENDING";
    if (statuses.length === 0)
        return "PENDING";
    if (statuses.length < enrolled)
        return "INCOMPLETE";
    if (statuses.some((s) => s === "RETURNED"))
        return "RETURNED";
    if (statuses.every((s) => s === "PUBLISHED"))
        return "PUBLISHED";
    if (statuses.every((s) => s === "APPROVED" || s === "PUBLISHED"))
        return "APPROVED";
...
async function assertTeacherAssignment(teacherId, subjectId, session) {
...
export async function upsertScoreWithWorkflow(input, actor) {
    ...
    if (enrollment.score &&
        ...
        throw new AppError(400, "This score is locked after approval/publish. Ask an admin to return it for correction.");
    }
    let gradeResult;
    try {
...
export async function bulkUpsertScores(input, actor) {
    requireSchoolId(actor);
    if (actor.role !== "TEACHER" || !actor.teacherId) {
        throw new AppError(403, "Only teachers can enter scores");
    }
    if (!input.scores.length)
        throw new AppError(400, "No scores provided");
    if (input.scores.length > 200)
        throw new AppError(400, "Too many scores in one request");
    const results = [];
    const errors = [];
    for (const row of input.scores) {
        try {
            const score = await upsertScoreWithWorkflow({ ...row, asDraft: input.asDraft }, actor);
            results.push(score);
        }
        catch (err) {
            errors.push({
...
export async function listResultSheets(params, actor) {
    ...
    const sheets = [...map.values()]
        ...
        scored: r.scored,
        missing: r.missing,
        completionPercent: r.enrolled > 0 ? Number(((r.scored / r.enrolled) * 100).toFixed(1)) : 0,
        status: sheetStatusFromScores(r.enrolled, r.statuses),
    }))
        .sort((a, b) => `${a.className}${a.subjectCode}`.localeCompare(`${b.className}${b.subjectCode}`));
    const counts = {
...
async function loadSheetEnrollments(schoolId, input) {
    const schoolClass = assertFound(await prisma.schoolClass.findFirst({ where: { id: input.classId, schoolId } }), "Class not found");
    const subject = assertFound(await prisma.subject.findFirst({ where: { id: input.subjectId, schoolId } }), "Subject not found");
    const enrollments = await prisma.enrollment.findMany({
        where: {
...
export async function getSheetDetail(input, actor) {
    const schoolId = requireSchoolId(actor);
    if (actor.role !== "ADMIN" && actor.role !== "TEACHER") {
        throw new AppError(403, "Not allowed");
    }
...
export async function transitionSheet(input, actor) {
    const schoolId = requireSchoolId(actor);
    if (actor.role !== "ADMIN")
        throw new AppError(403, "Only school admins can manage result approval");
    const { schoolClass, subject, enrollments } = await loadSheetEnrollments(schoolId, input);
    const scored = enrollments.filter((e) => e.score);
    if (scored.length === 0)
        throw new AppError(400, "No scores to process for this sheet");
    const now = new Date();
    if (input.action === "return") {
        const note = input.returnNote?.trim();
        if (!note)
            throw new AppError(400, "Return note is required");
    ...
    if (input.action === "approve") {
        ...
        const blocked = scored.filter((e) => e.score.status !== "SUBMITTED" && e.score.status !== "RETURNED" && e.score.status !== "APPROVED");
        // Allow re-approve of SUBMITTED/RETURNED; skip if already published
        const toApprove = scored.filter((e) => e.score.status === "SUBMITTED" || e.score.status === "RETURNED" || e.score.status === "DRAFT");
        if (toApprove.length === 0 && scored.every((e) => e.score.status === "APPROVED" || e.score.status === "PUBLISHED")) {
            return { message: "Already approved or published", status: "APPROVED" };
        }
        if (blocked.some((e) => e.score.status === "PUBLISHED") && toApprove.length === 0) {
            return { message: "Already published", status: "PUBLISHED" };
        }
        await prisma.score.updateMany({
    ...
    const notReady = scored.filter((e) => e.score.status !== "APPROVED" && e.score.status !== "PUBLISHED");
...
export async function getBroadsheet(params, actor) {
    const schoolId = requireSchoolId(actor);
    if (actor.role !== "ADMIN" && actor.role !== "TEACHER") {
        throw new AppError(403, "Not allowed");
    }
    const schoolClass = assertFound(await prisma.schoolClass.findFirst({
...
export async function getReportCard(studentId, params, actor) {
    const schoolId = requireSchoolId(actor);
    ...
    const visible = enrollments.filter((e) => {
        if (!e.score)
            return true; // show awaiting
        if (isStudent)
            return e.score.status === "PUBLISHED";
        return true;
    });
    ...
    const average = averageSource.length > 0
        ? Number((averageSource.reduce((s, e) => s + (e.score?.total ?? 0), 0) / averageSource.length).toFixed(2))
        : null;
    ...
    for (const sid of classmates.map((c) => c.id)) {
        ...
        if (usable.length === 0)
            continue;
        avgByStudent.set(sid, Number((usable.reduce((s, e) => s + e.score.total, 0) / usable.length).toFixed(2)));
    }
    ...
    const promotionHint = params.term === "THIRD" && average != null
...
export async function upsertTermComments(input, actor) {
    const schoolId = requireSchoolId(actor);
    const student = assertFound(await prisma.student.findFirst({ where: { id: input.studentId, schoolId } }), "Student not found");
    if (actor.role === "TEACHER") {
        if (!actor.teacherId)
            throw new AppError(403, "Teacher profile not found");
