...
router.get("/review", authorize("ADMIN", "TEACHER"), validate(sheetQuerySchema, "query"), async (req, res, next) => {
    try {
        const data = await resultWorkflow.listResultSheets(req.query, req.user);
...
router.get("/sheet", authorize("ADMIN", "TEACHER"), validate(sheetActionSchema, "query"), async (req, res, next) => {
    try {
        const data = await resultWorkflow.getSheetDetail(req.query, req.user);
        res.json({ success: true, data });
    }
    catch (error) {
        next(error);
...
router.get("/broadsheet", authorize("ADMIN", "TEACHER"), validate(broadsheetSchema, "query"), async (req, res, next) => {
    try {
        const data = await resultWorkflow.getBroadsheet(req.query, req.user);
        res.json({ success: true, data });
    }
    catch (error) {
        next(error);
...
router.get("/report-card/:studentId", validate(reportCardSchema, "query"), async (req, res, next) => {
    try {
        const data = await resultWorkflow.getReportCard(req.params.studentId, req.query, req.user);
        res.json({ success: true, data });
    }
    catch (error) {
        next(error);
